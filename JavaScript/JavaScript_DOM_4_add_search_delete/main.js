...
var input=document.querySelector("#addItemId");
var select=document.querySelector("#selectid");
var ul=document.querySelector(".items");
var search=document.querySelector("#searchid");
...
function searchItems(event){
	...
	var searchMe=event.target.value.toLowerCase();
	var array=Array.from(itemList);

	for(var i=0;i<array.length;i++){
		if(array[i].textContent.toLowerCase().indexOf(searchMe)!=-1){
			array[i].parentNode.style.display ="inline";
		}else{
...
function deleteEvent(event){
	...
	if(event.target.classList.contains("del")){
		if(confirm("Are you sure ?")){
			var delItemBox=event.target.parentNode;
			delItemBox.remove();
		}
